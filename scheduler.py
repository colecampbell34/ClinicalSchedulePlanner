...
#     # New: Track if this student is authorized for a Dual Chain
...
#         # (Student_ID, Block) -> "Red" | "Blue" | "Green" | "Orange" | "Purple"
...
#         def sort_key(s):
#             # PRIORITY 0: AUTHORIZED CONTINUITY
#             # Only if they were marked as a Chain Starter in the previous block
#             must_continue = False
#             if s.program == "Dual" and block in ["2B", "3B", "4B"] and s.is_chain_starter:
#                 must_continue = True
            
#             p_score = 0 if must_continue else 100
            
#             # Sub-priorities
#             if block in ["4A", "4B"]:
#                 if s.program == "Cardiac": p_score += 1
#                 elif s.program == "Dual" and s.cardiac_weeks_completed < 12: p_score += 2
#                 else: p_score += 4
#             else:
#                 # 2A-3B: Prioritize Duals with low weeks
#                 if s.program == "Dual": 
#                     p_score += (10 + s.cardiac_weeks_completed)
#                 else: 
#                     p_score += 99
#             return p_score
...
#                     prob = 0.5
#                     if s.clinic_blocks == 0 and block in ["3A", "3B"]: prob = 0.95
...
#                     # Only color Orange if it was a true Dual Chain
...
#                             # Reset flag (Chain complete)
#                             s.is_chain_starter = False
#                             return True
            
#             # --- 2. 4A/4B LOGIC ---
#             if block in ["4A", "4B"]:
#                 wants_clinic = any(c in s.choices for c in self.clinic_names)
#                 if wants_clinic:
#                      if self.try_clinic(s, block, force_no_repeat=force_no_repeat): return True
                
#                 if s.clinic_blocks == 0 and random.random() < 0.2:
#                      if self.try_clinic(s, block, force_no_repeat=force_no_repeat): return True

#                 if s.cardiac_weeks_completed >= 12:
#                     if self.try_site(s, block, general=True, force_no_repeat=force_no_repeat): return True
#                 else:
#                     if self.try_site(s, block, cardiac=True, force_no_repeat=force_no_repeat): return True
                
#                 return self.try_clinic(s, block, force_no_repeat=force_no_repeat)

#             # --- 3. 2A-3B LOGIC (Load Balanced) ---
...
#                 # LOAD BALANCER: 50% chance to prioritize General first in A-blocks
#                 # This pushes some Cardiacs to B-blocks to reduce clustering
...
#                                      allow_new_dual_start=can_start_dual): return True
#                 else:
#                     if self.try_site(s, block, cardiac=True, force_no_repeat=force_no_repeat, 
#                                      allow_new_dual_start=can_start_dual): return True
#                     if self.try_site(s, block, general=True, force_no_repeat=force_no_repeat): return True
...
#         filtered = []
#         for x in candidates:
#             # If site is Dual, can we take it?
#             # 1. If it's a NEW start (not last site) and disallowed (Block B) -> Skip
...
#             # QUOTA CONTROL: Starting a New Dual Chain
#             if s.program == "Dual" and site.is_dual:
#                 # If new start
#                 if site.name != last_site_name:
#                     # Only allow if Top Choice AND Quota not full
#                     is_top_choice = False
#                     try:
...
#                     if is_top_choice and self.dual_chains_assigned < self.MAX_DUAL_CHAINS:
#                         score -= 500 # Encourage
#                     else:
#                         score += 5000 # Suppress excess duals (treat as single)

#             try: score += s.choices.index(site.name)
#             except ValueError: score += 20 
...
#                 # Check if this authorizes a chain
#                 if s.program == "Dual" and site.is_dual and block in ["2A", "3A", "4A"]:
#                     # If score penalty didn't stop us, and we are here:
#                     # Are we creating a chain?
#                     if self.dual_chains_assigned < self.MAX_DUAL_CHAINS:
#                         # Authorize continuity for next block
...
#     def try_clinic(self, s, block, force_no_repeat=False):
#         def clinic_score(c):
#             score = 0
#             if c.name in s.assigned_sites: score += 100000
#             try: score += s.choices.index(c.name)
#             except ValueError: score += 20
...
#         if s.program == "General" or s.program == "Dual":
...
class RedesignedScheduler:
    def __init__(self, excel_path: str):
        ...
        # QUOTA TRACKER - RELAXED
        self.dual_chains_assigned = 0
        self.MAX_DUAL_CHAINS = 10 
    ...
    def schedule_block(self, block):
        all_active_students_for_block = [s for s in self.students if self.is_active(s, block)]
        
        # --- 1. GUARANTEED CHOICE PHASE ---
        zero_match_students = [
            s for s in all_active_students_for_block 
        ...
        def zero_match_sort_key(s):
            ...
            if block in ["4A", "4B"]:
                ...
                if s.program == "Dual": bucket += (10 + s.cardiac_weeks_completed)
                else: bucket += 50
        ...
        # 2a. Clinic attempts for General
    ...
    def place_student(self, s, block, force_no_repeat=False, is_zero_match_priority=False, only_choices=False):
        ...
        if s.program == "Dual":
            # --- 4A/4B LOGIC ---
            if block in ["4A", "4B"]:
                # If they are done Cardiac (>=12w), they should prioritize Clinic if they have none.
                if s.cardiac_weeks_completed >= 12:
                    
                    # PRIORITY CLINIC FILL
                    if s.clinic_blocks == 0:
                    ...
                    # Try Clinic (as secondary option)
                    if self.try_clinic(s, block, force_no_repeat=force_no_repeat, 
                                      is_zero_match_priority=is_zero_match_priority, only_choices=only_choices): return True
                    
                    return False # Banned from Cardiac
                else:
                    # Need Cardiac
                    if self.try_site(s, block, cardiac=True, general=True, force_no_repeat=force_no_repeat, 
                                     is_zero_match_priority=is_zero_match_priority, only_choices=only_choices): return True
                    return self.try_clinic(s, block, force_no_repeat=force_no_repeat, 
                                          is_zero_match_priority=is_zero_match_priority, only_choices=only_choices)
            ...
            if s.program == "Dual" and site.is_dual:
                if site.name != last_site_name:
                    ...
                    if is_top_choice and not ignore_choices_for_score and self.dual_chains_assigned < self.MAX_DUAL_CHAINS:
                        ...
                        # Reduced penalty from 5000 to 1000 to allow more natural duals if space exists
                        score += 1000 
